...
class App {
    constructor(container) {
        ...
        this.colors.map((color) =>
            this.followers.push(new Follower(this.svg, color))
        );
...
class Follower {
    ...
    makeCircle(point) {
        ...
        circle.setAttribute(
            "r",
            String(
                (Math.abs(point.direction.x) + Math.abs(point.direction.y)) * 1
            )
        );
    ...
    makeSquare(point) {
        let size =
            (Math.abs(point.direction.x) + Math.abs(point.direction.y)) * 1.5;
